...
var selected= new Array(48*7); //avalible=0, blacked out=1, booked=2
var nextWeek=0;
...
var t=Math.ceil(Math.abs(today - START_DATE) / (36e5/2));
    ...
    for(var i=0; i<48; i++)
    {
        ...
        for(var j=0; j<7; j++)
        {
            ...
            if(tempDate>today && tempDate<endDate)
            {
                ...
                if(selected[i+j*48-t + 48 + 336*nextWeek]==0)
                {
                ...
                else if(selected[i+j*48-t + 48 +  336*nextWeek]==1)
                {
                    timeTable=timeTable+' style="background: GREY">Blacked Out';
                }
                else if(selected[i+j*48-t + 48 + 336*nextWeek]==2)
                {
                    timeTable=timeTable+' style="background: RED">';
                }
                else
                {
                    timeTable=timeTable+'">';
